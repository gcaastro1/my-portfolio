        ...
        <>  
            ...
            <div className={`${styles.buttons} ${
                ...
                <motion.div className={`${styles.buttons__link} background`}>
                    { 
                        navItems.map((item) => (
                            <button className={styles.buttons__link__default} key={`button-${item.id}`} onClick={() => setActive(item.id)}>                       
                                <span className={active == item.id ? 'text__active' : 'text'}>
                                    {item.title}
                                </span>
                                {active == item.id && (
                                    <motion.div layoutId="nav-bg" className={`${styles.buttons__link__default__active} reversebg`}> 
                                    </motion.div>
                                )}
                            </button>
                        ))
